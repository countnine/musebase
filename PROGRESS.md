...
> **상태: windows-v0.20.0 (2026-09-13)** — 제어판 다듬기(곡명 판·좋아요 3상태 배지·아이콘 버튼·커버 따라잡기) + 오버레이 좌상단이 제어판 여닫기 버튼으로 + 번역 엔진에 OpenRouter + 오버레이 글꼴 지정. 가사 서버가 `/musebase` 아래로 옮겨져 **설정의 서버 주소 끝에 `/musebase`를 붙여야 한다**. 이전 0.19.0(제어판을 앨범 커버 창으로)·0.18.0(곡의 의미 보기) 포함.
