...
> **상태: windows-v0.20.1 (2026-09-13)** — 처음 트는 곡에서 서버가 아직 그 곡을 모를 때(404) 패널이 "서버에 연결하지 못했습니다"로 그리던 것 수정(+ 한 번 뜬 경고가 안 꺼지던 것). 0.20.0의 제어판 다듬기·오버레이 제어판 버튼·OpenRouter 번역·글꼴 지정 포함. 가사 서버가 `/musebase` 아래로 옮겨져 **설정의 서버 주소 끝에 `/musebase`를 붙여야 한다**.
...
## v0.20.1 수정 (Windows)
- **서버가 모르는 곡을 연결 오류로 표시하던 것** — 처음 트는 곡은 아직 아무도 올리지 않아 `GET /v1/song`이 404인데, 클라이언트가 404와 통신 실패를 똑같이 null로 돌려줘 패널이 "가사 서버에 연결하지 못했습니다"로 그렸다. 실측 타임라인: `14:37:44 조회 miss → 14:37:51 같은 기기가 업로드`(7초). 그 사이 새 곡마다 경고가 떴다.
  - 더 나빴던 건 **한 번 켜진 경고가 안 꺼진 것**이다. `_extrasFailed`를 내리는 곳이 "새 곡" 경로뿐이라, 5초 뒤 커버 따라잡기가 성공해 표지가 붙어도 ⚠와 문구가 곡이 끝날 때까지 남았다.
  - `GetExtrasAsync`가 `SongExtrasResult(ExtrasReach Ok/NotFound/Failed)`를 돌려주도록 바꿨다. 커버 재탐색·좋아요 토글은 사람이 누른 동작이라 그대로 뒀다(거기선 null이 곧 실패다). 패널은 `ApplyResult` 한 곳에서 매번 상태를 다시 계산한다.
  - 계약 문서에 "404는 오류가 아니다"를 근거와 함께 적었다. 테스트 3건 추가(404·500·401 구별).

## v0.20.0 추가분 (Windows)
- **제어판(앨범 커버 창) 다듬기 + 오버레이에서 제어판 열기(Windows)**
  - **곡명 자리에만 배경** — 창 아래쪽 전체에 그라데이션을 깔아 표지의 아래 절반이 늘 어두웠다. 이제 글자가 놓인 만큼만 둥근 판을 깔고 **우측 하단에 우측정렬**로 둔다.
  - **좋아요를 평상시에도 보여 준다** — 호버 층에만 있어 상태를 보려면 마우스를 올려야 했다. 우상단에 표시 전용 배지를 두고 **세 상태를 구별**한다: ♥ 켬 / ♡ 끔 / **! 확인 못함**. 모르는 것을 꺼진 하트로 그리면 사람이 눌러서 이미 켜 둔 것을 끄게 된다 — 그래서 `ToggleLove()`도 모르는 상태에서는 **끄는 쪽으로 보내지 않는다**(켜는 요청은 이미 켜져 있어도 해가 없고 응답으로 실제 상태를 받는다). 서버에 못 물어봤으면 ⚠와 함께 상태 줄에 이유를 적는다(예전에는 완전히 무음이었다).
  - **오버레이 좌상단 = 제어판 여닫기** — 그 자리의 재생 컨트롤은 같은 기능이 제어판에 있어 자리만 두 벌 쓰고 있었고, 정작 **오버레이에서 제어판으로 가는 길은 하나도 없었다**(트레이 메뉴·더블클릭·점프목록뿐). 우상단 자물쇠와 같은 형태의 `PanelButtonWindow`로 바꿨다(열려 있으면 녹색). `MediaControlWindow`는 삭제.
  - **커버가 나중에 생겨도 따라잡는다** — 처음 트는 곡은 서버에 가사 행이 없어 `GET /v1/song`이 404라 그 재생 내내 커버가 비어 있었다(가사를 올린 뒤에는 화면 상태가 안 바뀌어 다시 물을 계기가 없다). 5·15·40초에 되묻고 커버가 걸리면 멈춘다. 겸해서 **곡이 정말 바뀐 때만** 조회하게 고쳤다 — 예전에는 가사 상태가 바뀔 때마다(곡당 여러 번) 다시 받아 커버가 깜빡이고 GET이 중복됐다.
  - **버튼에서 네모를 걷어냄 + 툴팁** — 모든 버튼이 반투명 판 + 흰 테두리라 커버 위를 덮었다. 이제 글리프만 떠 있고 **호버할 때만** 은은한 원이 생긴다. 글꼴을 `Segoe Fluent Icons, Segoe MDL2 Assets`로 **명시**했다 — 지정이 없어 ⏮ ▶ 같은 문자가 폰트 폴백에 따라 두부(□)로 떨어질 수 있었다. 못 쓰는 버튼은 흐려진다(예전 템플릿엔 IsEnabled 트리거가 없어 눌리지만 않을 뿐 멀쩡해 보였다). 라벨은 툴팁으로 옮겼다.
  - **가사 서버에서 열기 버튼** — `{서버 주소}/song?key=…`를 브라우저로 연다. 키는 서버가 준 것만 쓴다(정규화 규칙이 서버 몫이라 앱이 만들면 다른 키가 나온다). 키나 서버 주소가 없으면 버튼이 사라진다.
- **번역 엔진에 OpenRouter 추가(코어 + Windows)** — 키 하나로 Claude·Gemini·GPT를 부를 수 있고 바꾸는 건 모델 문자열뿐이다. 기계 번역기와 실패 방식이 달라 방어가 하나 더 필요했다: **LLM은 줄 수를 지키지 않는다**(두 줄을 한 문장으로 합치거나 설명을 덧붙인다). 가사는 줄이 곧 타이밍이라 하나만 밀려도 화면 전체가 어긋나므로, 받은 줄 수가 다르면 **그 묶음을 통째로 버린다**(전부 null = 원문만 표시). 40줄/4000자 단위로 끊어 보내고 `temperature=0`, `max_tokens`는 반드시 싣는다(안 보내면 OpenRouter가 모델 최대치를 예약하려 들어 402). 언어는 코드 대신 이름으로 준다(`ZH-HANT` → `Traditional Chinese`). 설정 [번역] 탭에 모델 입력 칸(OpenRouter를 골랐을 때만). 테스트 11건.
- **오버레이 글꼴 지정(Windows)** — 지금까지 `OutlinedTextElement`에 `"Segoe UI"`가 하드코딩돼 있고 설정이 아예 없었다. 설정 [오버레이 스타일]에 편집 가능 콤보박스(설치된 글꼴 목록 + 직접 입력)와 미리보기를 넣었다. 패밀리 이름이 한글·영문 두 벌인 경우가 흔해 목록에 없는 이름도 받아야 한다 — 없는 이름은 `FontOf()`가 `"이름, Segoe UI"` 폴백 체인으로 만들어 조용히 내려간다(글자가 사라지지 않는다).

## 미배포 (다음 앱 릴리스 후보 — Android 0.5.0)
- **의미·좋아요가 기기 간에 어긋나던 것(서버+코어+Windows+Android)** — 서버에 의미가 있는데 앱에서 버튼을 누르면 **다시 만들어 덮어썼고**, 관리 화면에서 켠 좋아요를 앱에서 누르면 반대로 갔다.
  - **서버가 막는다.** `POST /v1/meaning`이 이미 있는 `ok` 의미를 확인 없이 새로 만들어 덮어썼다(`UpsertMeaning` 주석: "같은 key면 덮어쓴다"). 이제 **기본은 캐시 우선**이고 `force=1`일 때만 다시 만든다. 방어를 클라이언트에 두면 조회 실패·타임아웃·서킷 개방·`status≠ok`·응답 전 클릭에서 전부 무너진다 — 서버에 두어야 앱·웹·구버전이 함께 보호된다.
  - POST가 쓰는 자리를 조회와 같은 규칙으로 맞췄다. 조회는 느슨한 키로 형제 행까지 훑는데 저장은 현재 행 key에 해서, 표기가 갈린 곡에서 "조회는 있다는데 다시 만든다"가 됐다.
  - **좋아요는 누르기 전에 지금 값을 다시 받는다.** 서버는 토글이 아니라 절대값(`on=1|0`)을 받으므로, 곡 전환 때 한 번 받은 스냅샷을 뒤집어 보내면 의도와 반대로 간다(사용자에겐 "눌러도 안 바뀐다"). 그 사이 값이 바뀌어 있었으면 상태 줄에 알린다.
  - **재생성은 반드시 묻는다** — Windows는 MessageBox, Android는 [다시 만들기] 버튼 + 확인 창. 되돌릴 수 없고 비싼 호출이다.
  - **패널이 열려 있는 동안 30초마다 다시 확인한다.** 가사 서버에 방송 채널(WS/SSE)이 없고 넣으려면 서버·Windows·Android 세 곳에 수신을 붙여야 한다 — 폴링이 같은 문제를 훨씬 싸게 덮는다. 창이 안 보이거나 최소화면 쉰다.
  - 계약(`contracts/lyrics-api.md`)에 `force`와 캐시 우선 규칙을 적었다. 테스트 2건 추가(407개 통과). **서버 재배포 필요**.
...
## 서버 (2026-09-13 배포 완료 — 앱 릴리스와 무관하게 운영 중)
- **Spotify 라이브러리에도 좋아요 반영(서버)** — 좋아요를 켜고 끄면 Last.fm과 Spotify 양쪽에 반영된다. Last.fm 계정 연결 구조를 그대로 본떴다(`app_settings`에 갱신 토큰, 대시보드 카드, 논스 쿠키).
  - **2026년 2월 정책 변경을 정면으로 맞았다.** ① `PUT /me/tracks`는 폐기됐고 `PUT /me/library?uris=spotify:track:…`(삭제는 `DELETE`)로 통합됐다 — 옛 주소는 토큰·스코프가 맞아도 403이다. ② Development Mode가 **앱 소유자에게 Premium 구독을 요구**하고 인가 사용자를 5명으로 제한한다(Extended quota는 법인·250K MAU 요건이라 개인에겐 안 열린다). ③ 검색 `limit` 상한이 50 → 10. 셋 다 테스트로 못 박았다.
  - **콜백 주소를 앱 대시보드에 미리 등록해야 한다**(Last.fm은 요청할 때 넘길 수 있었다) — 안 적어 두면 승인이 조용히 실패하므로 관리 화면이 넣어야 할 주소를 그대로 띄운다.
  - 가사에는 트랙 ID가 없어 **제목·아티스트로 검색**해 찾고 커버 아트와 같은 판정(`MeaningMatch.IsSameSong`)을 통과한 것만 쓴다 — 검색은 리믹스·노래방 음원을 곧잘 위로 올린다. 찾은 URI는 기억하고 **못 찾은 것도 기억한다**(`user_version` 9의 `spotify_uri`·`spotify_at`).
  - **한쪽만 실패할 수 있다** — 어느 쪽이 안 됐는지 그대로 알린다(`LoveResult`). Last.fm이 되고 Spotify만 실패하면 `502`가 아니라 `200`에 `spotifyKnown: false`다. 절반이라도 반영된 것을 통째로 오류로 만들면 앱이 화면을 되돌려 오히려 사실과 멀어진다.
  - ⚠ 갱신 토큰이 DB에 들어가 **백업 파일에 Spotify 쓰기 자격증명이 함께 담긴다**(Last.fm 세션 키와 같다) — `deploy/README.md` 13절에 명시.
  - 테스트 10건. **서버 재배포 필요**(env 미설정이면 기능 전체가 꺼진 채 동작이 이전과 같다).
- **서버를 `/musebase` 아래로 옮김 + 관리 화면 개편** — 한 테일넷 호스트에 다른 서비스를 함께 얹을 수 있게 관리 화면 `/admin` → `/musebase`, API `/v1` → `/musebase/v1`로 통째로 옮기고 **옛 경로는 남기지 않았다**. 경로 리터럴 84군데를 `Routes.Base`/`Routes.Api` 한 곳으로 모았다(관리자 쿠키 `Path`와 라우트 접두사가 어긋나면 로그인은 되는데 로그아웃이 쿠키를 못 지우는 식으로 조용히 깨진다). CSP·`no-store` 미들웨어는 `/musebase/v1`을 제외한다.
  - **앱 코드는 무변경** — 클라이언트가 서버 주소에 `v1/…`을 상대 경로로 붙이므로 설정 주소를 `https://호스트/musebase`로 바꾸면 된다. 다만 **올린 직후 모든 기기가 끊기므로** 배포하면서 PC·폰 설정을 함께 고쳐야 한다. 관리자 쿠키 Path가 바뀌어 브라우저는 한 번 다시 로그인한다.
  - **검색 필터를 칩으로** — 드롭다운(전체/있음/아직 없음 3개)을 칩으로 바꾸고 각 칩에 건수를 박았다(전체 · 의미 있음 · 아직 안 만듦 · 자료 부족 · 자료 없음 · 생성 실패 · ♥ 즐겨찾기). 칩은 **서로 겹치지 않는다** — 합이 전체를 넘으면 숫자를 믿지 않게 되므로 "아직 안 만듦"은 `m.key IS NULL`(일괄 생성이 실제로 처리할 대상)로 두고, 기존 `none`(ok가 아닌 곡 전부)은 옛 주소용으로만 남겼다. 건수는 `SearchCounts()` 한 번의 쿼리로 센다(이전에는 COUNT 쿼리가 하나도 없었다).
  - **즐겨찾기 칩 = Last.fm 좋아요** — `user.getLovedTracks`로 목록을 통째로 받아 `song_links.loved`에 적어 둔다(`user_version` 8). 곡마다 물으면 한 화면에 수백 번을 부르게 된다. 받다가 실패하면 **아무것도 바꾸지 않는다** — 부분 반영은 못 받은 곡을 해제된 것처럼 보이게 한다.
  - **페이징** — 200건에서 말없이 잘려 201번째 곡부터는 볼 방법이 없었다. 30건씩 나누고 `31–60 · 전체 1123건`과 이전/다음을 붙였다.
  - **목록 가독성** — 짧은 열은 전부 `nowrap`(기기 이름이 IP면 두 줄로 접혀 표가 들쭉날쭉했다), 곡·아티스트만 폭 제한 + 말줄임(전체 값은 `title=`). **아티스트가 링크**가 되어 그 아티스트의 곡을 한 번에 본다.
  - **삭제·광고 표시에 확인창** — 소스 전체에 `confirm(`이 0건이라 되돌릴 수 없는 동작이 한 번의 클릭으로 실행됐다. 기존 제출 스크립트에 `data-confirm`을 이어 붙였다(스크립트는 여전히 하나뿐이고 CSP 해시가 자동으로 따라온다). 좋아요 버튼만 `margin`이 남아 옆 버튼보다 눌려 보이던 것도 맞췄다.
  - **의미 생성 LLM을 화면에서 고른다** — 지금 쓰는 엔진·모델을 대시보드에 띄우고 제공자·API 키·모델을 그 자리에서 바꾼다. `MeaningSettings`가 `server.env` 위에 `app_settings` 값을 덮고, 바뀌면 서비스를 다시 만들어 **재시작 없이** 다음 생성부터 적용된다(예전에는 SSH로 파일을 고치고 재시작해야 했다). 키는 끝 네 글자만 보여 주고 빈 칸으로 저장하면 유지한다. ⚠ **화면에 넣은 키는 DB에 평문으로 저장되어 백업에 들어간다**(Last.fm 세션 키와 같다) — 그게 싫으면 `server.env`만 쓴다.
  - **Tunefind 링크를 곡 페이지로** — `/search?q=아티스트+곡명`은 검색이 흩어져 잘 안 맞았다. 이제 `/song/{아티스트}/{곡명}` 슬러그를 만든다(`mazzy-star/look-on-down-from-the-bridge`). 규칙 생성이 안전한 이유는 Last.fm과 같다 — 틀리면 "없는 페이지"가 뜰 뿐 Musixmatch처럼 조용히 다른 곡으로 넘어가지 않는다. 한글 등 슬러그를 못 만드는 이름은 **곡명만으로** 검색에 물러난다.
  - **`GET /musebase/v1/song`에 `key` 추가** — 앱이 `{서버 주소}/song?key=…`로 관리 화면을 바로 열 수 있게. 키 규칙은 전적으로 서버 몫이라 클라이언트가 조립할 수 없다. 필드 추가라 구버전 앱과 호환(`contracts/lyrics-api.md` 갱신).
  - 테스트 374개 통과(신규 22건). **서버 재배포 필요**.
...
## v0.2.0 추가분
- 전체화면 앱 감지 시 오버레이 자동 숨김 (`FullscreenDetector`, 1s 폴링, 이동 모드 중엔 억제 안 함). **오버레이가 있는 모니터만 본다** — 듀얼 모니터에서 옆 화면의 전체화면(RDP 등)까지 세면 가리지도 않는데 가사창이 사라진다
