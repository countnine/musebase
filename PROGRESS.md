...
> **상태: windows-v0.21.0 (2026-09-14)** — 제어판이 위치·크기를 기억하고 긴 곡명을 줄여 두 줄로 접는다. 오버레이에 숨기기(✕) 버튼이 생기고 숨김이 유지된다. 서버에 이미 있는 곡의 의미를 다시 만들어 덮어쓰던 것과 좋아요가 반대로 가던 것 수정. 설정 백업·복원(비밀번호로 잠가 내보내기) 추가. 가사 서버가 `/musebase` 아래에 있으므로 **설정의 서버 주소 끝에 `/musebase`가 붙어 있어야 한다**.
