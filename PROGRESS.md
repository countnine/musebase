...
> **상태: windows-v0.21.1 (2026-09-14)** — 설정 가져오기가 오류로 끝나던 것과, 가져올 때 이 PC의 API 키·토큰이 지워지던 것 수정(0.21.0에서 들어간 백업 기능의 구멍). 제어판이 위치·크기를 기억하고 긴 곡명을 줄여 두 줄로 접는다. 오버레이에 숨기기(✕) 버튼이 생기고 숨김이 유지된다. 서버에 이미 있는 곡의 의미를 다시 만들어 덮어쓰던 것과 좋아요가 반대로 가던 것 수정. 설정 백업·복원(비밀번호로 잠가 내보내기) 추가. 가사 서버가 `/musebase` 아래에 있으므로 **설정의 서버 주소 끝에 `/musebase`가 붙어 있어야 한다**.
> 재개 방법: "이어서"라고 입력하면 아래 백로그부터 진행.

## Android 0.8.0 (2026-09-18)
- **곡의 의미를 가사 맨 위 카드로** — 예전에는 상단 ⓘ 아이콘 → 팝업이라 무엇을 하는 버튼인지 알기 어렵고, 의미가 있어도 눌러 봐야만 알 수 있었다. 이제 곡이 바뀌거나 화면에 돌아오면 서버에 조용히 묻고(`GET` — 만들지 않으니 비용 없음) 있으면 가사 맨 위에 세 줄 카드로 보인다(누르면 펼쳐 출처까지). 없으면 같은 자리에 [의미 알아보기]. 상단 버튼은 글자 **[의미]** — 의미가 있는 곡이면 색이 채워진다. 다시 만들기는 펼친 카드에서만, 확인을 거친다(0.7.0 이후 머지된 확인 창 포함).
- **API 키·서버 토큰 보호** — 일반 SharedPreferences에 평문이었고 `allowBackup`으로 Google 백업에 실렸다. 이제 Android Keystore(AES-256-GCM, 키는 기기 밖으로 안 나감)로 암호화해 별도 파일에 두고, 백업 규칙으로 그 파일만 뺀다. 0.7.0의 평문은 첫 실행 때 옮긴 뒤 지운다(옮긴 것을 `Commit`으로 확정한 뒤에만).
- **고정 릴리스 키** — 0.7.0까지는 빌드한 PC의 디버그 키로 서명돼, 그 PC가 없어지자 업데이트를 낼 수 없었다. 이제 CI가 고정 릴리스 키(GitHub 시크릿)로 서명한다. **0.7.0에서 올 때만 한 번 제거 후 설치**(설정 초기화)가 필요하고, 이후로는 덮어쓰기로 올라간다.
- **CI가 Android를 빌드한다** — 솔루션 밖이라 지금까지 한 번도 빌드되지 않았다. 러너 기본 SDK 10은 지원이 끝난 `net8.0-android`를 거부해(NETSDK1202) SDK 8로 고정했다. `net8.0-android`는 지원 종료 — 다음 큰 작업 때 새 .NET으로 올릴 것.
...
## 서버·코어 — 의미 생성이 실패한 이유를 남긴다 (2026-09-18)
- **계기(실측 장애 두 번)**: 관리화면 [의미 가져오기]가 ① "일시적인 오류입니다(쿼타·네트워크)"로 끝났는데 실제로는 **OpenRouter 잔액 소진(402)**이었고, ② 키를 바꾸고 무료 모델로 옮기자 "생성에 실패했습니다(키를 확인하세요)"가 됐는데 실제로는 **계정 정책상 무료 모델 차단(404)**이었다. 서버가 공급자 응답 본문을 버리고 로그도 남기지 않아 둘 다 curl로 재현해서야 원인을 알았다. ②는 `failed` 행으로 DB에 박제돼 백필이 그 곡을 건너뛰게 만들기까지 했다.
  - `MeaningWriteResult`가 상태코드와 공급자 오류 문장(200자)을 싣는다. 429·5xx·타임아웃만 `retry`, **그 밖의 4xx(402 잔액·401/403 키·404 모델·400 모델 id)는 새 상태 `config`** — 기다려도 안 풀리고 곡 탓도 아니므로 **저장하지 않는다**. `failed`로 저장되는 것은 200인데 본문이 빈 경우처럼 그 곡의 문제일 때뿐이다.
  - 서버가 엔진 실패(`failed`·`retry`·`config`)마다 `journalctl`에 곡·엔진·모델·이유를 한 줄 남긴다. 관리화면 문구에 이유가 붙고(`HTTP 402 · This request requires more credits`), 일괄 생성은 `config`에서도 멈춘다.
  - 앱: `202 {"status":"config"}`는 `Unavailable`로 옮긴다("이 서버에서는 만들 수 없다" — 앱에서 할 일이 없다). 구버전 앱은 모르는 값이라 "만들지 못했다"로 보인다(해 없음). 계약 문서 갱신.
  - **숨은 타임아웃**: 공용 `MeaningHttp.Client.Timeout`이 30초라 OpenRouter writer의 60초(최대 180초) 예산을 몰래 잘랐다 → 느린 모델은 늘 "일시적 오류". 안전망을 4분으로 올리고 만료는 호출마다의 CTS에 맡긴다(모든 호출자가 CTS를 건다 — 확인함).
  - 테스트 17건 추가(상태 분류, 이유 추출·자르기, 두 엔진의 본문 보존, 빈 키, 저장 안 함 규칙, 타임아웃 안전망).

## v0.21.1 수정 (Windows)
- **설정 가져오기가 실패하던 것 + 가져올 때 키가 지워지던 것** — 실사용에서 바로 걸렸다.
  - `ImportSettings()`가 끝에서 `DialogResult = true`를 썼는데 설정 창은 `Show()`로 뜨는 **모달이 아닌 창**이다. WPF는 이때 "Window를 만들고 대화 상자가 표시된 후에만 DialogResult를 설정할 수 있습니다"로 던진다 — 설정은 이미 반영·저장된 뒤라 **파일은 제대로 들어갔는데 화면에는 실패로 보였다**. [저장] 버튼과 같은 길(`_onSaved()` → `Close()`)로 바꿨다. 겸해서 백업에 든 UI 언어가 창을 닫은 뒤 적용된다(예전에는 다음 실행까지 안 바뀌었다).
  - 더 무거운 쪽: `ApplyBackup`이 모든 속성을 반사로 덮어써서, 백업에 **안 담긴** 비밀값까지 `null`로 밀어 버렸다. 암호문(`*Enc`)은 애초에 백업에 담기지 않으므로(다른 PC에서 못 푼다) 비밀번호 없이 만든 백업을 가져오면 이 PC의 키가 **평문·암호문 양쪽에서** 사라졌다 — 되돌릴 방법이 없는 손실이다. 이제 비밀값은 `KeptOnImport`로 지키고 **백업에 실제로 값이 들어 있을 때만** 덮어쓴다. 빈 칸은 "지우라"가 아니라 "안 담겼다"는 뜻이다.
  - 내보낸 파일이 메모장에서 그대로 읽히는 것은 **정상**이다 — 평문 설정만 보이고 키·토큰은 `secrets` 블록의 base64 암호문이다(테스트가 평문이 파일에 안 남는 것을 확인한다).

## v0.21.0 추가분 (Windows)
- **제어판 위치·크기 기억 + 곡명·아티스트·상태 줄 다듬기** — 매 실행마다 화면 정중앙에 360×360으로 새로 떴다(CenterScreen). 이제 자리를 기억하고, 저장된 값이 없으면 작업영역 **우하단에서 24 DIP** 띄운다. 저장은 끌기를 놓을 때와 닫을 때이고 **화면 밖 보정을 저장보다 먼저** 한다(오버레이 쪽의 순서 버그를 되풀이하지 않으려고).
  - **긴 곡명**: 판 너비가 300 고정이고 줄바꿈이 없어 창을 키워도 같은 자리에서 잘렸다. 이제 판이 창을 따라 넓어지고 제목은 17 → 15 → 13으로 필요한 만큼만 줄었다가 그래도 넘치면 두 줄로 접힌다(WPF TextBlock에 `MaxLines`가 없어 `LineHeight × 2` + `BlockLineHeight`로 제한).
  - **호버 층 겹침**: `top`과 우상단 창 버튼이 `veilGrid`의 같은 행을 공유하는데 오른쪽 여백이 없어 긴 제목이 버튼 밑으로 깔렸다. 버튼 폭만큼 비웠다. (평상시 층의 곡명 판은 우하단이라 원래 겹치지 않았다 — 보고와 원인이 달랐던 부분.)
  - **아티스트**: 12 → 14pt, `Dim`(#8A93A2) → `#C3CAD6`. Dim은 상태·설명용이라 읽으라고 두는 글자엔 너무 흐렸다.
  - **셋째 줄 중복**: `LocalizeStatus`의 `{track}`이 `TrackInfo.ToString()` = `"아티스트 - 제목"`이라 바로 위 줄이 한 번 더 나오고 정작 소스 정보가 잘렸다. `LocalizeStatusShort`를 더해 제어판에는 소스·품질만 준다(트레이 툴팁은 곡명이 필요하므로 그대로).
  - **좋아요 버튼이 두부(□)**: `_love`만 아이콘 글꼴로 `♡`(U+2661)를 그렸다. **그 글꼴에 없는 글리프다** — Segoe Fluent Icons·MDL2 둘 다 조회해 확인했고 ``·``로 바꿨다. 같은 파일의 `TextButton`은 이미 이 함정을 알고 글꼴을 벗겨 쓰고 있었다.
  - **열 때 컨트롤이 펼쳐지던 것**: WPF가 창 활성화 때 첫 포커스 가능 요소(= 호버 층 버튼)에 포커스를 주고 `GotKeyboardFocus`가 그걸 받았다. 마지막 입력이 키보드일 때만 열게 해 탭 진입은 살렸다. 겸해서 마우스가 이미 창 위면 바로 열리게 했다.
- **오버레이 숨김이 유지되게 + 버튼을 모서리로** — `MiniWindow`가 복원·활성화마다 `ReviveOverlay()`를 불러, 숨겨 둬도 **제어판을 클릭하는 순간 다시 켜지고 저장까지** 됐다. 되살리기는 명시적 경로에서만 한다. 또 `ReviveVisible()`이 일시정지 억제를 덮어써 멈춘 가사가 다음 재생까지 떠 있었다 — 이제 건드리지 않고, 되살릴 때 지금 재생 상태를 다시 반영한다.
  - 버튼 배치: 제어판 좌상단 → **좌하단**, 자물쇠 우상단 → **우하단**, **숨기기(✕) 우상단 신규**. 표시 판단·좌표가 버튼마다 흩어져 있던 것을 `UpdateOverlayButtons()` 한 곳으로 모았다. 이동 모드에서는 자물쇠만 남긴다.
  - `KeepOnScreen()`이 가상 화면(작업표시줄 포함) 대신 **그 창이 놓인 모니터의 작업영역**을 본다(`Services/MonitorWorkArea.cs` — `SystemParameters.WorkArea`는 주 모니터만 준다). `WM_EXITSIZEMOVE`에서 보정을 저장보다 먼저 한다 — 반대 순서라 화면 밖 좌표가 저장됐다.
  - 설치 직후 기본 위치(주 모니터 중앙 하단·작업표시줄 위)는 **이미 그렇게 되어 있어** 그대로 뒀다.

- **설정 백업·복원(Windows)** — 새 PC에 설치하면 DeepL 키·가사서버 주소·토큰을 일일이 다시 넣어야 했다. 설정 [일반] 탭에 [설정 내보내기…]·[가져오기…]를 넣었다.
  - **파일 복사로는 안 된다.** 비밀값 5개가 DPAPI `CurrentUser`로 잠겨 있어 다른 PC·계정에서 조용히 null이 된다. 그래서 내보낼 때 잠금을 풀고 **사람이 정한 비밀번호로 다시 봉인**하고(PBKDF2-SHA256 210,000회 + AES-GCM), 가져올 때 풀어 그 PC의 DPAPI로 다시 잠근다. 반복 횟수는 서버 관리자 비밀번호와 같은 값을 쓴다.
  - 비밀번호를 비우면 **키를 아예 빼고** 내보낸다(잠그지 않은 키를 파일에 흘리는 것보다 낫다). 내보내기는 두 번 받아 맞춰 본다 — 오타가 굳으면 나중에 열 방법이 없다.
  - **전부 되거나 아무것도 안 되거나**다. 비밀번호가 틀리면 평문 설정도 반영하지 않는다.
  - 기기마다 달라야 하는 값(`TelemetryClientId`, 오버레이·패널 좌표)은 덮어쓰지 않는다. `*Enc`(이 PC에서만 풀리는 암호문)는 아예 담지 않는다.
  - 봉인·해제는 `Musebase.Core/Settings/SettingsBackup.cs` — 플랫폼 설정 타입을 모르게 JSON 조각으로 주고받아 WPF에 묶이지 않고 시험된다(테스트 13건).
- **설정 저장이 암호문을 지우던 버그(Windows)** — `Save()`가 무조건 `*Enc = Protect(평문)`을 썼는데, 복호에 실패해 평문이 null이면 암호문까지 null이 되고 `WhenWritingNull` 때문에 키가 파일에서 빠졌다. **다른 PC·계정에서 settings.json을 한 번 열어 보기만 해도 키가 영영 사라졌다.** 복호 실패는 "값이 없다"가 아니라 "여기서는 못 읽는다"일 뿐이다 — 이제 기존 암호문을 그대로 둔다. 백업과 무관하게 필요한 수정이다.
- **의미·좋아요가 기기 간에 어긋나던 것(서버+코어+Windows+Android)** — 서버에 의미가 있는데 앱에서 버튼을 누르면 **다시 만들어 덮어썼고**, 관리 화면에서 켠 좋아요를 앱에서 누르면 반대로 갔다.
  ...
  - 계약(`contracts/lyrics-api.md`)에 `force`와 캐시 우선 규칙을 적었다. 테스트 2건 추가.
  - **서버는 2026-09-14에 배포했다.** Windows 몫은 0.21.0에 나가고, Android의 [다시 만들기] 확인 창은 0.8.0에 나갔다 — 다만 서버가 캐시 우선이라 구버전 Android도 이미 덮어쓰지 않는다.

## Android 0.5.0~0.7.0에 나간 것 (예전 "미배포" 목록 — 모두 출시됨)
