...
- **이중언어 표시** — 원문과 번역을 2단으로. 가사 소스의 번역을 우선하고, 없으면 기계번역 —
  **MyMemory**(무료·키 불필요), **DeepL**, **Google Cloud Translation**, **LibreTranslate**(키/엔드포인트 입력형),
  **OpenRouter**(LLM — 모델을 직접 고름).
...
기본 상태에서는 대상 언어 번역이 없는 줄을 **MyMemory**(무료·키 불필요)로 자동 번역합니다.
더 높은 품질을 원하면 **설정**에서 엔진을 고르고 그 엔진의 키를 넣으세요(키 입력란은 선택한 엔진을
따라갑니다) — [DeepL API](https://www.deepl.com/pro-api) 무료 키(월 50만 자) 또는
[Google Cloud Translation](https://cloud.google.com/translate)(Google Cloud 콘솔에서 API 사용 설정 후
API 키 발급, 사용량 기반 유료). 선택한 엔진이 실패(할당량·네트워크)하면 무료 엔진으로 자동 전환하는
옵션도 있습니다. 번역은 줄 단위로 캐시됩니다(같은 곡은 1회만 번역). 키는 암호화되어 저장됩니다.

**OpenRouter**를 고르면 키 하나로 여러 LLM을 부를 수 있습니다(모델 칸에
`anthropic/claude-opus-5` 같은 이름을 넣습니다). 다만 기계 번역기와 실패 방식이 다릅니다 —
LLM은 두 줄을 한 문장으로 합치거나 설명을 덧붙일 수 있는데, 가사는 줄이 곧 타이밍이라 하나만
밀려도 화면 전체가 어긋납니다. 그래서 **받은 줄 수가 원문과 다르면 그 묶음은 번역 없이 원문만**
보여 줍니다(어긋난 자막보다 낫습니다).

### 글꼴

설정 → [오버레이 스타일]에서 글꼴을 고르거나 이름을 직접 넣을 수 있습니다(설치된 글꼴 목록이
뜨지만, 패밀리 이름이 한글·영문 두 벌인 경우가 있어 직접 입력도 됩니다). 없는 이름이면 조용히
Segoe UI로 내려가므로 글자가 사라지지 않습니다.

### 가사 서버 (선택)

여러 기기를 쓴다면 가사·번역 캐시를 서버 한 대에 모아 함께 쓸 수 있습니다. 한 기기가 찾아 둔 가사를
다른 기기가 그대로 받아 **검색과 유료 번역을 반복하지 않습니다.** 설정 [소스] 탭에 주소와 토큰을
넣으면 되고, 넣지 않으면 아무것도 달라지지 않습니다(서버가 꺼져 있어도 평소대로 동작).

→ [가사 서버 사용 가이드](docs/lyrics-server-guide.md) · [설치·배포](src/Musebase.Server/deploy/README.md)
