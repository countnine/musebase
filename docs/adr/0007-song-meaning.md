...
새 가사가 올라올 때 자동으로 만들지 않는다. 사람이 누르는 자리만 둔다.

**2026-09-13 변경**: 누르는 자리를 **앱에도** 둔다(`POST /v1/meaning`, Android의 [의미 만들기]).
지키려던 것은 "자동 생성 금지"였지 "관리자 화면 독점"이 아니었다 — 듣다가 궁금해진 순간에
서버 관리자 화면을 따로 열어야 하는 것은 그 원칙과 무관한 불편이었다. 원칙은 그대로다:

- 자동 호출은 여전히 없다(재생·조회는 절대 생성을 유발하지 않는다).
- 같은 곡에 요청이 겹치면 **한 번만 만들고 결과를 나눠 준다**(`MeaningGenerator`의 in-flight 합치기).
  기기 여러 대에서 동시에 눌릴 수 있는 만큼 이게 없으면 같은 곡에 LLM을 두 번 쓴다.
- 광고로 표시된 제목과 서버에 없는 곡은 만들지 않는다 — 토큰을 쓸 이유가 없다.
- 비용이 걱정되면 `MUSEBASE_MEANING_ALLOW_CLIENT=0`으로 앱 경로만 막는다(조회는 그대로).

바뀐 것은 **비용을 낼 수 있는 사람이 늘었다**는 점이다. 토큰을 가진 기기는 모두 생성을 시킬 수
있으므로, 토큰이 새면 가사 오염만이 아니라 LLM 비용까지 샌다. 테일넷 안 개인 서버라 받아들인다.

관리자 화면은 그대로 단건 버튼 + 일괄 백필을 갖는다.
