...
넘어간다 — `user@host:/path`(scp, 테일넷 이름 가능) 또는 `gs://버킷/경로`(gcloud). 원격 복사가 실패하면
유닛이 **failed**로 남는다(`systemctl status musebase-backup`).
...
- **앱(Android)의 [의미 만들기]** — 듣다가 그 자리에서 만든다. 같은 곡에 요청이 겹치면
  서버가 한 번만 만들고 결과를 나눠 준다.
- **생성은 사람이 누를 때만 일어난다.** 자동 생성은 두지 않았다 — 쿼타·비용이 예측 가능해야 하고
  실패가 조용히 쌓이면 안 되기 때문이다.

앱에서 만드는 것을 막고 싶으면(비용을 관리자만 쓰게):

```
MUSEBASE_MEANING_ALLOW_CLIENT=0      # 기본은 켬. 조회(GET)는 이 값과 무관하게 계속 된다
```
