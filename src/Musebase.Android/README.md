# Musebase.Android (Phase 2 — 가사 엔진 조립 + 앱 내 동기 가사 표시)
...
| `MusebaseApp.cs` | 커스텀 `Application` — `LyricsEngineFactory.Create`로 엔진 1회 조립(화면 회전에도 유지). 소스=레지스트리 전체(개인용), 번역=libretranslate(무키), 대상 언어=기기 로케일, 캐시=`FilesDir/translations.db`, 텔레메트리=Noop |
| `MainActivity.cs` | 앱 내 가사 UI — 검색 상태 문구 + 현재 줄(크게) + 번역(아래), `StateChanged`/`StatusChanged` 구독. 스파이크의 권한 안내·곡 정보 1초 갱신 표시 유지 |
| `AndroidManifest.xml` | 템플릿(+INTERNET 권한). `<service>`(BIND_NOTIFICATION_LISTENER_SERVICE)/`<activity>`/`<application android:name>`은 C# 특성에서 생성·병합 |
...
가사 오버레이(다른 앱 위 표시)·카라오케 채움·DeepL 키 입력 UI는 다음 단계.
...
5. 가사 영역: "가사 검색 중…" → "가사: <소스> (품질 …)"로 바뀌고, 재생 위치에 맞춰
   ...
   번역 줄은 LibreTranslate 공용 인스턴스 상태에 따라 지연/생략될 수 있다(무키 무료).
