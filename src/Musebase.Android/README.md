...
| 테두리 링 **줄 수** | 오버레이가 **지금 보이는가** | 보이면 같은 굵기·밝기의 링이 안쪽에 하나 더 생겨 **두 줄** / 안 보이면 한 줄 |
...
- "보이는가"의 근거는 `_bandExpanded`가 아니라 **실제 표시 여부**(`_bandVisible`)다 — 펼쳐 뒀어도
  일시정지·무가사면 밴드는 보이지 않으므로, 펼침 플래그로 칠하면 버블이 거짓말을 한다.

**펼침/접힘은 사용자가 정한 상태다** — 버블 탭(또는 이동 모드 진입) 말고는 아무도 바꾸지 않는다.
곡이 바뀔 때 접던 동작이 있었는데(0.3.0), 곡이 넘어갈 때마다 버블을 다시 눌러야 해서 없앴다.
이동 모드는 옮기려면 밴드가 보여야 해서 강제로 펼치지만, 끝나면 **들어가기 전 상태로 되돌린다**.
설정 저장(`ActionRefreshDisplay`)도 접지 않는다 — 접는 건 버블 모드로 **막 들어왔을 때 한 번**뿐이다.
