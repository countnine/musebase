...
| 채널 | 뜻 | 표기 |
|---|---|---|
| 테두리 링 | 가사 상태 | 검색 중 = 흐린 트랙 위로 **악센트 호가 회전**(플레이스토어 다운로드 링과 같은 감각, 1.1초/바퀴) / 찾음 = 가사 색(기본 흰색) 실선 / 못 찾음·무곡 = 회색 |
| 안쪽 채움 | 오버레이가 **지금 보이는가** | 보이면 링 색으로 **채우고 음표를 어두운색으로 반전** / 안 보이면 어두운 채움 + 밝은 음표 |
