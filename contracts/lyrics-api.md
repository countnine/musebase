...
### 의미 만들기 (`POST /musebase/v1/meaning?title=&artist=&force=`)

**기본은 캐시 우선이다.** 이미 만들어 둔 의미(`status=ok`)가 있으면 **만들지 않고 그대로**
`200`으로 돌려준다. 다시 만들려면 `force=1`을 붙인다 — 그건 **기존 글을 덮어쓰므로**
사람이 확인했을 때만 보낸다.

> 예전에는 확인 없이 새로 만들어 덮어썼고, 막는 것이 클라이언트의 "이미 있나" 판단 한 겹뿐이었다.
> 그 판단은 조회 실패·타임아웃·서킷 개방·`status≠ok`·응답 전 클릭에서 모두 무너진다 —
> 그래서 방어를 서버로 옮겼다(앱·웹·구버전이 함께 보호된다).
...
**서버가 그 곡을 모르면 `404`다. 이것은 오류가 아니다** — 아직 아무도 올리지 않은 새 곡이면
정상이고, 보통 몇 초 뒤 그 기기가 직접 찾아 올린다. 클라이언트는 이 `404`를 **연결 실패와
구별해야 한다**(실측: 조회 미스 7초 뒤에 업로드가 끝났는데 그 사이 화면에 "서버에 연결하지
못했습니다"가 떴다). 404면 커버·좋아요 자리를 조용히 비우고, 잠시 뒤 다시 물어보면 된다.
